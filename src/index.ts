...
${formattedRanks}`;
  ...
  async function c(session: Session, guess: string) {
    ...
    if (!allWords.includes(guess)) {
      return await sendMsg(session, '词典中没有这个词！');
    }
...
正确答案：${gameInfo[0].answer}`;
      ...
      if (playerInfo.length === 0) {
        ...
        await ctx.database.set('ciyi_rank', {userId: session.userId}, {
  ...
  async function mrtz(session: Session) {
    ...
    while (!isNone && gameInfo[0].guessedWords.includes(answer)) {
      answer = random.pick(questionList);
    }
...
解释：
右侧数字：用户输入的词语与答案的“含义相似程度”排名，值越小，越接近答案。

输入词语的左右两侧提示：与用户输入的词语含义最接近的两个词语中的各一个字，“？”为未知字占位符。`;
    return await sendMsg(session, msg);
  }
